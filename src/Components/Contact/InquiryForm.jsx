...
export default function InquiryForm() {
  ...
  function handleSubmit(event) {
    ...
    const message = [
      "Hello VELISQA, I want to submit a bespoke inquiry.",
      `Name: ${formData.get("name") || "Not provided"}`,
      `Email: ${formData.get("email") || "Not provided"}`,
      `Inquiry: ${formData.get("inquiryType") || "Not provided"}`,
      ...
      <form className="space-y-9" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-9 md:grid-cols-2">
          <div className="flex flex-col gap-2">
            ...
            <input className={fieldClass} name="name" placeholder="Gautam Adani" type="text" />
          </div>
          <div className="flex flex-col gap-2">
            <label className="label-stitch uppercase tracking-wider text-[#514347]">Email Address</label>
            <input className={fieldClass} name="email" placeholder="client@luxury.com" type="email" />
          </div>
        </div>
        <div className="flex flex-col gap-2">
          <label className="label-stitch uppercase tracking-wider text-[#514347]">Nature of Inquiry</label>
          <select className={fieldClass} name="inquiryType" defaultValue="Custom Engagement Design">
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea className={`${fieldClass} min-h-32 resize-none`} name="message" placeholder="Describe your vision or the collection of interest..." />
        </div>
