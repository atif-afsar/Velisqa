-- Migration: Add Pincode, Coupon Support, and Guest Checkout Support
-- Run in Supabase SQL Editor

-- 1. Add discount_amount and coupon_code to public.orders
alter table public.orders add column if not exists discount_amount numeric(12, 2) not null default 0.00 check (discount_amount >= 0);
alter table public.orders add column if not exists coupon_code text;

-- 2. Make user_id nullable for Guest Checkout
alter table public.orders alter column user_id drop not null;

-- 3. Recreate create_manual_payment_order to support discount, coupon_code, and guest checkout
...
begin
  ...
  if v_coupon_code = 'SAVE10' then
    v_discount_amount := round(v_subtotal * 0.10, 2);
  elsif v_coupon_code = 'VELISQA5' then
    v_discount_amount := round(v_subtotal * 0.05, 2);
  elsif v_coupon_code = 'FREE50' then
    v_discount_amount := least(50.00, v_subtotal);
  end if;
